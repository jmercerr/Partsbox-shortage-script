...
@params 
	...
	- Timestamps: dictionary of timestamps for the four time periods used
@returns 
...
@params 
	- sorted_stock: list of parts without positive stock values and values from moving parts in warehouse
@returns 
	- refined_stock: list of parts that have had valid stock history within the past year 
'''
def remove_empty_stock(sorted_stock):
	part_entry = 0 
	refined_data = []

	for part in sorted_stock: 
		data = sorted_stock[part_entry]
		valid = True

		try:
			stock_data = sorted_stock[part_entry]['stock']
		except KeyError:
			valid = False
